...
if uploaded_file:
    ...
    if st.session_state.resume_text is None:
        ...
        st.success("Resume parsed. You can now use either tab below.")

resume_ready = st.session_state.resume_text is not None

tab1, tab2 = st.tabs(["🔍 Auto Search (Adzuna)", "✍️ Paste JD Manually"])
        ...
        if st.session_state.search_results:
            ...
            for idx, r in enumerate(st.session_state.search_results):
                with st.container(border=True):
                    ...
                    if st.button("✍️ Generate cover letter", key=f"auto_gen_btn_{idx}"):
                    ...
                    if letter_key in st.session_state:
                        st.text_area("Generated cover letter", st.session_state[letter_key],
    ...
    if not resume_ready:
        ...
        if st.session_state.manual_result:
            ...
            with st.container(border=True):
                ...
                if st.button("✍️ Generate cover letter", key="manual_gen_btn"):
                    with st.spinner("Extracting keywords and generating cover letter..."):
                ...
                if "manual_letter" in st.session_state:
                    st.text_area("Generated cover letter", st.session_state.manual_letter,
